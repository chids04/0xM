...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const { milestoneId, ownerUid, fee } = data;
    ...
    if(ownerUid === currentUserUid){
      return createErrorResponse("INVALID_ACTION", "Owner cannot sign their own milestone", 403)  
    }
    ...
    if (!milestoneCids) {
      return new Response(
        JSON.stringify({
          message: "I CID not found",
          errorCode: "MILESTONE_CID_NOT_FOUND"
        }),
        { status: 404, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    if(bal < signMilestoneFee){
      return createErrorResponse("INSUFFICIENT_BALANCE", "Insufficient balance to sign milestone", 403)
    }

    const callData = trackerContract.interface.encodeFunctionData("signMilestone", [
      ownerAddress,
      milestoneId,
    ]); 
